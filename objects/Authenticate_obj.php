...
class authenticate {
    ...
    public function get_users() {
        $prep_stmt = "SELECT username, firstName, lastName, email, failedLogin, lastLogin, status, created FROM users";
        ...
        if ($stmt) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                while ($stmt->fetch()) {
                    ...
                    $users[] = ['username' => $username, 'firstName' => $firstName, 'lastName' => $lastName, 'email' => $email, 'failedLogin' => $failedLogin, 'lastLogin' => $lastLogin, 'status' => $status, 'created' => $created, 'groups' => $groups];
                    $groups = null;
                }
            } else {
                $stmt->close();
     ...
     * @param bool optional Flag to return array (true) or update object (false)
     *
     ...
     *
     * @param array user All user information
     * @param bool super Is super user, so can change user status
     *
     * @return bool  Success flag 
     */
     public function update_user($user,$super=false) {
        // Update user
        if ($super) {
            // Update user record
    ...
    function login_check($username, $sessionHash, $remember) {
        ...
        // @TODO Uptionally add user-agent and ip checks 
     ...
     *
     * @param bool admin [optional] If true check admin rights
     *
     * @return bool success flag
     */    
    public function authorisation_check($admin = false) {
        ...
        if (isset($_SESSION['username'], $_SESSION['sessionHash'])) {
            ...
            try {
                ...
                echo json_encode(array('success' => false, 'message' => 'Authorization failure', 'code' => 401));
    ...
    private function sendConfirmationMail() {
        ...
        $to = $this->email;
        $subject = 'Valideer gebruiker voor ' . TITLE;
	...
	// @todo Andere SMTP mail functie gebruiken
    ...
    public function change_password($username, $password, $password1, $password2, $token=null) {
        if (strlen($password1) < 8) { // @TODO get this value from configuration
            throw new Exception('Password to short, minimum of 8 characters required');
        }
        if ($password1 != $password2) {
            throw new Exception('Password strings not equal');
        }
    ...
    public function sendUnlockMail() {
        ...
        $to = $this->email;
        $subject = 'Ontgrendel account voor ' . TITLE;
